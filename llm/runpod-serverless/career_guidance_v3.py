#!/usr/bin/env python3
"""
Career Guidance Model - V3 Aligned with checkpoint-8400 (96.9% gen-audit)

Key Quality Gates:
1. Resume ATS rule: "'ATS Keywords:' present" OR ≥5 commas (relaxed from ≥8)
2. Response seeding: ONLY resume_guidance, recruiting_strategy, ats_keywords get "1. "
3. HR decoding: temperature=0.3, repetition_penalty=1.0, max_tokens per domain
4. Intent classification false positive fixes
5. Salary range regex for all currencies (SGD, SEK, etc.)
6. Over-sanitization prevention
"""
...
class ResponseValidator:
    ...
    def validate_resume_guidance(response: str) -> Tuple[bool, List[str]]:
        ...
        # Relaxed ATS Keywords rule: "'ATS Keywords:' present" OR ≥5 commas
        has_ats_line = 'ats keywords:' in response.lower()
        comma_count = response.count(',')
        if not has_ats_line and comma_count < 5:
            issues.append(f"Missing 'ATS Keywords:' line and insufficient commas ({comma_count} < 5)")
...
Be specific and relevant. No metadata. Keep 80-120 words."""
    ...
    def build_domain_prompt(domain: str, question: str) -> str:
        ...
        # Seed ONLY resume_guidance, recruiting_strategy, ats_keywords with "1. "
        SEEDED_DOMAINS = {"resume_guidance", "recruiting_strategy", "ats_keywords"}
        if domain in SEEDED_DOMAINS:
            prompt += "<|im_start|>assistant\n1. "
        elif domain == "job_resume_match":
            # Match domain starts with score
            prompt += "<|im_start|>assistant\nScore: "
        else:
            prompt += "<|im_start|>assistant\n"
